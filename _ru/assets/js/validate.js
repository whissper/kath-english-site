...
function showCharLength() {
	...
	document.getElementById('countTA').value = "Символов осталось: " + textLength;
}
...
function sendData(event) {
	...
	if (window.jQuery) { 		
		if ( !allFieldsAreValid() ) {
			document.getElementById('sendStatusField').innerHTML = '<div class="alert alert-danger" role="alert">Проверьте, пожалуйста, что все поля заполнены корректно!</div>';
			return false;
		}
 	}
	
	document.getElementById('sendStatusField').innerHTML = '<div class="alert alert-primary" role="alert">Сообщение отправляется...</div>';
		...
		doAJAX({
			...
			success: function() {
				...
				if (messageData.isvalid) {
					...
					document.getElementById('sendStatusField').innerHTML = '<div class="alert alert-success" role="alert">Сообщение отправлено</div>';
				} else {
					//alert('Something went wrong! try to send it again');
					document.getElementById('sendStatusField').innerHTML = '<div class="alert alert-danger" role="alert">Что-то пошло не так! Попробуйте отправить сообщение заново</div>';
				}
			},
			error: function(){
				//alert('Error occured! Try again later');
				document.getElementById('sendStatusField').innerHTML = '<div class="alert alert-danger" role="alert">Произошла ошибка! Попробуйте отправить сообщение позже</div>';
			}
		});
	} else {
